...
export default function PriceCard({ data: { header, name, description, priceWithUnit, buttonText = "Start Free Trial", points } }) {
  return (
      ...
      <Box>
        ...
        >
          <Link activeClass="active" to={"contact"} spy={true} smooth={true} offset={-70} duration={500}>
            <Button variant={header ? "primary" : "whiteButton"} aria-label={buttonText}>
              {buttonText}
            </Button>
          </Link>
        </Box>
